...
- The script is currently only meant to be used with PCIe designs and will not work with other Transceiver protocols
...
- The script will work with PCIe interfaces spread across one Transceiver quad (x1, x2 or x4), two Transceiver quads (x8) or four Transceiver quads (x16)
...
- The script offers two modes of operation:
  - auto: This mode is intended to be used in a block design which has a single PCIe PHY IP instance (i.e. one PCIe interface in the entire block design). This tutorial will demonstrate how to use auto mode.
  - manual: This mode can be used in a block design which has two or more PCIe PHY IP instance to update one PHY interface at a time. Please refer to the manual mode usage details documented in the comments of the Tcl script ([migrate_to_gtwiz_subsystem.tcl](./migrate_to_gtwiz_subsystem.tcl)).  

- The script does not currently support designs where the PCIe interface is sharing a Transceiver quad with another PCIe or non-PCIe Transceiver protocol (e.g. The script will not work if two PCIe x2 interfaces are implemented in the same Transceiver quad or if one PCIe x2 interface and a different Transceiver protocol are implemented using the remaining 2 Transceivers in the same Transceiver quad)
- The script and the tutorial below are not applicable to designs which use the CPM4 or CPM5 blocks to implement PCIe in Versal (which do not require migration)
<p>
...
3. When prompted, click on Generate to Generate the IP Output Products (Out of context per IP). Also, click on OK when the following dialog box appears: "Out-of-context module run was launched for generating output products." Note that it could take 15+ minutes to complete this step.

4. Once the IP Output Products have been generated, select the qdma_0.xci file under Design Sources, right-click and choose "Open Example Design...". This will launch another instance of Vivado where [the Example Design project will be generated](https://docs.amd.com/r/en-US/pg344-pcie-dma-versal/Customizing-and-Generating-the-Example-Design?tocId=39mP~VpnoogCIkkfJCzDJQ). It will take a few minutes for this step to be completed.
...
6. In the Run Block Automation dialog which appears, the default options can be used. Click OK. Block Automation should complete quickly.
...
8. Click on Generate Device Image. A prompt will appear to launch synthesis and implementation, click Yes.

9. Allow the project to be run through synthesis and implementation. It will take a while to complete this step. Once completed, a prompt will appear to Open the Implemented Design, click OK.
...
This step is optional, it will just highlight the old IP in the existing project which will be updated in this tutorial.
...
2. If you double-click to open the QDMA Support IP block (**<span style="color:green">qdma_0_support</span>**), you will see the Legacy Transceivers Wizard IP core mapping to 2 Transceiver quads (**gt_quad_0** and **gt_quad_1**) since this is an x8 design. Note that prior to 2024.2, the name of this IP core was **Versal ACAPs Transceivers Wizard**:
...
1. Open the existing Vivado 2024.1 PCIe Design using Vivado 2024.2. Vivado will display the following message (referencing [AR#000036830](https://adaptivesupport.amd.com/s/article/000036830?language=en_US)). Click on the **Upgrade** button.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;![AR000036830](images/PCIeIP_Migration_to_GTWizSubsystem_02_2024-2_AR000036830.png)

2. The following message will appear indicating that newer versions of some of the IP cores in the design are available in Vivado 2024.2. Click on the **Report IP Status** button.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;![ReportIPStatus Prompt](images/PCIeIP_Migration_to_GTWizSubsystem_03_2024-2_ReportIPStatusPrompt.png)

3. Vivado will provide a report of all the IP cores which need to be upgraded in the design. Click on the **Upgrade Selected** button. In the prompt which follows, click **OK**.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;![ReportIPStatus](images/PCIeIP_Migration_to_GTWizSubsystem_04_2024-2_ReportIPStatus.png)

4. The following prompt will be displayed asking to proceed with Upgrading the IP cores. Click **OK**. The IP cores will then be upgraded - this process could take a few minutes and a progress bar will appear as the IP cores are updated.
...
#5, #6 and #7 below are optional instructions, just to familiarize yourself further with the IP which will be migrated.
...
6. If you double-click to open the QDMA Support IP block (**<span style="color:green">qdma_0_support</span>**), you will see the Legacy Transceivers Wizard IP core again mapping to 2 Transceiver quads (**gt_quad_0** and **gt_quad_1**) since this is an x8 design. In 2024.2, the Legacy IP core has been renamed to **Versal Adaptive SoC Transceivers Wizard**:
